...
export class Cacher {
  ...
  setCache(key, value) {
    ...
    this.cacheMap.set(JSON.stringify(key), JSON.stringify(response))
  ...
  hasCache(key) {
    return new Promise((resolve, reject) => {
      let exists = this.cacheMap.has(JSON.stringify(key))
      resolve(exists)
    })
  }
  ...
  getCache(key) {
    return new Promise((resolve, reject) => {
      let data = JSON.parse(this.cacheMap.get(JSON.stringify(key)))
      resolve(data)
    })
  }
